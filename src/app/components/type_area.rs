...
pub fn TypeArea(cx: Scope, send: Action<String, Result<String, ServerFnError>>) -> impl IntoView {
    ...
    view! {cx,
        <div class="h-24 w-full fixed bottom-0 flex justify-center items-center p-5 bg-white border-t border-gray-300 rounded-full">
            <form class="w-full flex justify-center items-center gap-4" on:submit=move |e| {
                e.prevent_default();
